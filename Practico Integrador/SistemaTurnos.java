...
public class SistemaTurnos {
    ...
    public boolean cancelarTurno(String idTurno) {
        ...
        System.out.printf("  Fecha: %s%n", turno.getFechaHora().format(DateTimeFormatter.ofPattern("dd/MM/yyyy HH:mm")));
    ...
    static class PlannerRecordatorios {
        ...
        private void percolateUp(int i) {
            while (i > 0) {
                int parent = (i - 1) / 2;
                if (heap.get(i).fecha.compareTo(heap.get(parent).fecha) >= 0) break;
                swap(i, parent);
                i = parent;
            }
        }
    ...
    public void programarRecordatorios() {
        ...
        planner.programar(new Recordatorio("R1", LocalDateTime.now().plusDays(1), "32045982", "Control próxima semana"));
    ...
    public void consolidarAgendas() {
        ...
        while (i < agenda1.size()) consolidada.add(agenda1.get(i++));
        while (j < agenda2.size()) consolidada.add(agenda2.get(j++));
    ...
    static class Accion {
        enum TipoAccion { AGENDAR, CANCELAR, REPROGRAMAR }
        TipoAccion tipo;
        Turno turno;
    ...
    static class AgendaConHistorial {
        private List<Accion> pilaUndo;
        private List<Accion> pilaRedo;
    ...
    class MinHeapQuirofanos {
        private ArrayList<Quirofano> heap;
        ...
        private void heapifyUp(int idx) {
            while (idx > 0) {
                int padre = (idx - 1) / 2;
                if (heap.get(idx).compareTo(heap.get(padre)) >= 0) break;
    ...
    class MinHeapCirugias {
        private ArrayList<Cirugia> heap;
    ...
    public void planificadorQuirofano() {
        ...
        ArrayList<Map.Entry<String, Integer>> listaMedicos = 
            new ArrayList<>(cirugiasXMedico.entrySet());
